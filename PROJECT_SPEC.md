...
|---|---|---|---|
| **Web scraping เว็บราชการ** | 🔴 สูง | HTML เปลี่ยน / โดนบล็อค IP / ผิด ToS → detector พัง | cache raw file, มี manual-download fallback, ไม่ยิงถี่, เขียน parser ให้ fail loud — **CONFIRMED จริงใน Step 6 (live-tested กับเว็บจริง หลังแก้ปัญหา TLS ของเครื่องแล้ว):** CGD ใช้ requests+BeautifulSoup ได้ปกติ (200, HTML จริง, ยืนยันด้วยการรัน `src/detect.py` ตัวจริงกับเว็บจริงสำเร็จ). **OCSC มี 2 ปัญหาแยกกัน**: (1) โดน Cloudflare managed JS challenge บล็อคเป็นบางครั้ง (ไม่ใช่ทุกครั้ง — เจอจริงครั้งเดียว, บันทึกไว้ที่ `tests/fixtures/ocsc_cloudflare_challenge.html`) และ (2) **แม้ผ่านด่าน Cloudflare ได้ (HTTP 200 หน้าเว็บจริง) รายการไฟล์ก็ยังไม่อยู่ใน static HTML อยู่ดี** — ถูก render ด้วย JS filter widget (`admin-ajax.php`), ยืนยันแล้วว่าไม่มี link `.xlsx/.pdf/.zip` เลยแม้แต่อันเดียวในหน้า 369KB ที่ดึงมาได้จริง (`tests/fixtures/ocsc_real_page_no_file_links.html`). สรุปคือผ่าน Cloudflare ได้ก็ยังสแครปไม่ได้อยู่ดี — ต้องรัน JavaScript จริงซึ่งอยู่นอก scope. OCSC จึงใช้ manual-check fallback แทน (อัปเดตปีละครั้งอยู่แล้ว ผลกระทบต่ำ). ดู `src/detect.py` |
