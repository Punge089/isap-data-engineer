...
|---|---|---|---|
| **Web scraping เว็บราชการ** | 🔴 สูง | HTML เปลี่ยน / โดนบล็อค IP / ผิด ToS → detector พัง | cache raw file, manual-download fallback, ไม่ยิงถี่, parser fail loud. **CONFIRMED (Step 6, live-tested):** CGD ใช้ requests+BeautifulSoup ได้ปกติ (200 จริง). **OCSC สแครปไม่ได้ 2 ทาง**: (1) Cloudflare JS challenge บางครั้ง, (2) แม้ผ่าน Cloudflare ได้ ไฟล์ก็ยังไม่อยู่ใน static HTML (JS-rendered widget) — ยืนยันแล้วทั้งคู่, รายละเอียดเต็มดู `src/detect.py` + HANDOFF.md. ใช้ manual-check fallback แทน (ปีละครั้ง ผลกระทบต่ำ) |
